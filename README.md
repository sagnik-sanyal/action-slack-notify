...
| ENABLE_ESCAPES           | -                                                     | If set to `true`, will enable backslash escape sequences such as `\n`, `\t`, etc. in the message. Note: This only works for custom messages set in the `SLACK_MESSAGE` field and not for the default messages (Or other fields such as `SLACK_TITLE`), generated by the action.                                             |
